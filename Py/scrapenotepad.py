...
from tkinter import StringVar, IntVar, scrolledtext, END, messagebox, filedialog, LEFT
import requests
...
def close_note():
    question = messagebox.askyesno("Close Note", "Are you sure you want to end your note    ?")
    if question == 1:
        root.destroy()
...
def open_note():
    open_name = filedialog.askopenfile()
...
webpage_image = ImageTk.PhotoImage(Image.open('globe.png'))
webpage_input = tkinter.Entry(menu_frame, width=20, font='Cambria, 16', bg='#684934', borderwidth=5, justify=LEFT)
webpage_button = tkinter.Button(menu_frame, image=webpage_image, command=import_web_text)
webpage_input.grid(row=2, column=3, padx=5, pady=5)
webpage_button.grid(row=1, column=3, padx=5, pady=5)
